...
# Make all text bold in evaluation figures (titles, labels, dates, legends, etc.)
plt.rcParams.update({
    "font.weight": "bold",
...
def create_statistical_visualizations(df, overall_metrics, output_dir):
    ...
    # Create figure with multiple subplots (2 rows x 3 columns = 6 panels, but only 5 used)
    # Add extra space at top for treatment-N legend
    fig = plt.figure(figsize=(18, 13))
    gs = fig.add_gridspec(2, 3, hspace=0.4, wspace=0.4, top=0.92, bottom=0.08)
    ...
    if np.sum(mask) > 0:
        ...
        # Add treatment labels near points (larger, bold, dark for readability)
        for i in range(len(yield_obs)):
            if mask[i]:
                label = f"T{int(treatments[i])}"
                ax1.annotate(
                    label,
                    (yield_obs[i], yield_sim[i]),
                    fontsize=11,
                    fontweight='bold',
                    alpha=1.0,
                    # Slight offset so label is outside the dot
                    xytext=(6, 4),
    ...
    # 2. Model Efficiency by Treatment - Just numbers on x-axis (1, 2, 3...)
    ...
    if treatment_efficiency:
        ...
        ax4.set_title('b) Model Efficiency by Treatment', fontsize=12, fontweight='bold')
    ...
    # 3. Grain Nitrogen: Simulated vs Observed - WITH treatment labels on points
    ...
    if np.sum(mask_gn) > 0:
        ...
        ax6.set_title('c) Grain Nitrogen: Simulated vs Observed', fontsize=12, fontweight='bold')
    ...
    ax7.set_title('d) Model Performance Metrics (Yield)', fontsize=12, fontweight='bold')
    ...
    if summary_data:
        ...
        ax8.set_title('e) Summary Statistics', fontsize=12, fontweight='bold', pad=20)
    ...
    fig.suptitle(f'DSSAT N-Wheat Crop Growth Model Validation - Duernast {wheat_type} Experiment {EXPERIMENT_YEAR} under Different Nitrogen Treatments',

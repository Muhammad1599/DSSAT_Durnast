...
# Make all text bold in figures (titles, labels, dates, legends, etc.)
...
def read_cultivar_parameters():
    """Read cultivar parameters from .CUL file based on cultivar used in WHX file"""
    
    # Determine the directory structure based on experiment year
    # Handle both cases: running from project root or from experiment directory
    current_dir = Path.cwd()
    
    if EXPERIMENT_YEAR == 2015:
        # Try paths relative to current directory first
        whx_path = Path(f'input/{FILE_PREFIX}.WHX')
        cul_path = Path('Genotype/WHAPS048.CUL')
        
        # If not found, try from project root
        if not whx_path.exists():
            whx_path = Path(f'DUERNAST2015/input/{FILE_PREFIX}.WHX')
        if not cul_path.exists():
            cul_path = Path('DUERNAST2015/Genotype/WHAPS048.CUL')
    elif EXPERIMENT_YEAR == 2017:
        # Try paths relative to current directory first
        whx_path = Path(f'input/{FILE_PREFIX}.WHX')
        cul_path = Path('Genotype/WHAPS048.CUL')
        ...
        # If not found, try from project root
        if not whx_path.exists():
            whx_path = Path(f'DUERNAST2017/input/{FILE_PREFIX}.WHX')
        if not cul_path.exists():
            cul_path = Path('DUERNAST2017/Genotype/WHAPS048.CUL')
    else:
        print(f"[WARNING] Unknown experiment year {EXPERIMENT_YEAR}, cannot locate cultivar file")
        return None
    
    # First, read WHX file to find which cultivar is used
    cultivar_code = None
    cultivar_name = None
    
    if not whx_path.exists():
        print(f"[WARNING] WHX file not found at {whx_path}, trying current directory...")
        whx_path = Path(f'{FILE_PREFIX}.WHX')
    
    if whx_path.exists():
        try:
            with open(whx_path, 'r', encoding='utf-8', errors='ignore') as f:
                lines = f.readlines()
            
            in_cultivars_section = False
            for i, line in enumerate(lines):
                if '*CULTIVARS' in line:
                    in_cultivars_section = True
                    continue
                if in_cultivars_section and line.strip().startswith('@'):
                    # Header line, skip
                    continue
                if in_cultivars_section and line.strip() and not line.strip().startswith('!'):
                    # Data line - format: "C CR INGENO CNAME"
                    parts = line.strip().split()
                    if len(parts) >= 4:
                        cultivar_code = parts[2]  # INGENO
                        cultivar_name = parts[3] if len(parts) > 3 else parts[2]
                        break
                if in_cultivars_section and line.strip().startswith('*'):
                    # Next section started
                    break
        except Exception as e:
            print(f"[WARNING] Could not read cultivar from WHX file: {e}")
    
    if not cultivar_code:
        print("[WARNING] Could not determine cultivar code from WHX file")
        return None
    
    print(f"[INFO] Found cultivar code: {cultivar_code} ({cultivar_name})")
    
    # Now read .CUL file to get parameters
    if not cul_path.exists():
        print(f"[WARNING] .CUL file not found at {cul_path}")
        return None
    
    try:
        with open(cul_path, 'r', encoding='utf-8', errors='ignore') as f:
            lines = f.readlines()
        
        # Find the header line to understand column positions
        header_line_idx = -1
        for i, line in enumerate(lines):
            if '@VAR#' in line and 'VRNAME' in line:
                header_line_idx = i
                break
        
        if header_line_idx == -1:
            print("[WARNING] Could not find header line in .CUL file")
            return None
        
        # Parse header to understand column positions
        # Format: @VAR#  VRNAME.......... EXPNO   ECO#  VSEN PPSEN    P1    P5 PHINT  GRNO MXFIL STMMX SLAP1
        header = lines[header_line_idx].strip()
        
        # Find cultivar data line
        cultivar_params = None
        for i in range(header_line_idx + 1, len(lines)):
            ...
            # Split by whitespace - cultivar code should be first column
            parts = line.split()
            if len(parts) >= 2 and parts[0] == cultivar_code:
                # Found the cultivar! Parse parameters
                # Format: VAR# VRNAME EXPNO ECO# VSEN PPSEN P1 P5 PHINT GRNO MXFIL STMMX SLAP1
                # Note: VRNAME may have spaces, so we need to be careful
                # The format is: VAR# (first), then VRNAME (may have spaces), then rest
                try:
                    # Find where the cultivar code ends and parameters start
                    # After VAR#, next fields are: VRNAME (variable length), EXPNO, ECO#, then numeric params
                    # Strategy: Find VAR# at start, then skip VRNAME (which may have spaces), 
                    # then find numeric values starting from VSEN
                    
                    # Start from the beginning - VAR# is first
                    if parts[0] != cultivar_code:
                        continue
                    
                    # VRNAME might be multiple words, so we need to find where numbers start
                    # Look for the first numeric value after VAR# - that should be EXPNO or ECO#
                    # Actually, EXPNO might be "." so we need to find VSEN (which is always a float)
                    
                    # The format is fixed-width: VAR# VRNAME (padded) EXPNO ECO# VSEN PPSEN P1 P5 PHINT GRNO MXFIL STMMX SLAP1
                    # VRNAME may have spaces but is padded with dots, so splitting by whitespace works
                    # After VAR# (index 0), we need to find where VSEN starts
                    # VSEN is typically at index 4 (after VAR#, VRNAME parts, EXPNO, ECO#)
                    # But VRNAME can be 1-3 words, so we need to find VSEN by looking for a float value
                    
                    # Find VSEN - it's the first float value after ECO# (which is typically a code like "IB0001")
                    # Strategy: Find the position of VSEN by looking for the pattern: code, code, float
                    # Actually simpler: VSEN is always the 5th numeric-like field (VAR#, VRNAME words, EXPNO, ECO#, VSEN)
                    # But EXPNO can be ".", so let's find VSEN by position
                    
                    # Count fields: VAR# (0), then VRNAME (1-N), then EXPNO (can be "."), then ECO# (code), then VSEN (float)
                    # Let's find VSEN by looking for the first float after ECO#
                    vsen_idx = None
                    for idx in range(2, min(len(parts), 10)):  # VSEN should be within first 10 fields
                        try:
                            val = float(parts[idx])
                            # Check if this could be VSEN (typically 0.0 to 4.0)
                            if 0.0 <= val <= 5.0:
                                # Check if previous field looks like ECO# (starts with letters/numbers)
                                if idx > 1 and (parts[idx-1].startswith('IB') or parts[idx-1].startswith('AW') or 
                                               parts[idx-1].startswith('SP') or parts[idx-1].startswith('WI') or
                                               parts[idx-1] == 'DFAULT' or parts[idx-1].startswith('999')):
                                    vsen_idx = idx
                                    break
                        except ValueError:
                            continue
                    ...
                    if vsen_idx is None:
                        # Fallback: assume standard format - VSEN at index 4
                        vsen_idx = 4
                        if len(parts) <= vsen_idx:
                            print(f"[WARNING] Line too short: {line[:100]}")
                            continue
                    
                    # VRNAME is everything between VAR# and EXPNO
                    # EXPNO is typically at vsen_idx - 2 (before ECO#)
                    expno_idx = vsen_idx - 2
                    varname = ' '.join(parts[1:expno_idx]).strip(' .') if expno_idx > 1 else parts[1] if len(parts) > 1 else ''
                    
                    # Parameters start at VSEN (vsen_idx)
                    param_start = vsen_idx
                    
                    cultivar_params = {
                        'VAR#': parts[0],
                        'VRNAME': varname.strip(),
                        'VSEN': float(parts[param_start]) if len(parts) > param_start else None,
                        'PPSEN': float(parts[param_start + 1]) if len(parts) > param_start + 1 else None,
                        'P1': float(parts[param_start + 2]) if len(parts) > param_start + 2 else None,
                        'P5': float(parts[param_start + 3]) if len(parts) > param_start + 3 else None,
                        'PHINT': float(parts[param_start + 4]) if len(parts) > param_start + 4 else None,
                        'GRNO': float(parts[param_start + 5]) if len(parts) > param_start + 5 else None,
                        'MXFIL': float(parts[param_start + 6]) if len(parts) > param_start + 6 else None,
                        'STMMX': float(parts[param_start + 7]) if len(parts) > param_start + 7 else None,
                        'SLAP1': float(parts[param_start + 8]) if len(parts) > param_start + 8 else None,
                    }
                    break
                except (ValueError, IndexError) as e:
                    print(f"[WARNING] Error parsing cultivar parameters: {e}")
                    print(f"  Line was: {line[:100]}")
                    continue
        
        if cultivar_params:
            print(f"[OK] Loaded cultivar parameters for {cultivar_code}")
            return cultivar_params
        else:
            print(f"[WARNING] Cultivar {cultivar_code} not found in .CUL file")
            return None
            
    except Exception as e:
        print(f"[WARNING] Could not read .CUL file: {e}")
        import traceback
        traceback.print_exc()
        return None
    ...
    try:
        ...
        for run_num in range(1, min(16, len(run_sections))):
                ...
                try:
                    ...
                    if len(parts) >= 35:
                        ...
                        data_rows.append({
                            ...
                            'WFPD': wspd,
                            'WFGD': wsgd,
                            'SLFT': slft,  # Soil water factor for leaves (leaf-based water stress)
                            'NFTD': nftd,
                            'NSTD': nstd,
                        })
                except (ValueError, IndexError) as e:
            ...
            if data_rows:
                ...
                # Daily water stress index (for time series visualization)
                # WSGD: Water stress grain development (0=severe stress, 1=optimal)
                # Convert to stress index: 1 - WSGD (0=no stress, 1=severe stress)
                df['daily_water_stress_index'] = 1.0 - df['WFGD']  # Higher = more stress
                # Cumulative water stress (matching R script approach)
                # R script uses: sum(WSGD) for cumulative water stress
                # For cumulative stress over time, use cumsum(WSGD)
                # Note: Higher WSGD = better conditions, so cumulative WSGD: higher = better conditions (lower stress)
...
def create_comprehensive_visualization(treatments_data, phenology_stages, consensus_stages, 
                                     weather_data=None, nitrogen_data=None, observed_data=None, n_levels=None, cultivar_params=None):
    ...
    # No need to adjust height since parameters are in corner
    fig, axes = plt.subplots(12, 1, figsize=(18, 36))
    
    # Add cultivar parameters at the top if available
    if cultivar_params:
        # Create a horizontal text box with cultivar parameters (short form)
        def format_param(value, fmt='.2f'):
            """Format parameter value, handling None"""
            if value is None:
                return 'N/A'
            try:
                return f"{value:{fmt}}"
        ...
        # Format as horizontal single line
        cultivar_code = cultivar_params.get('VAR#', 'N/A')
        cultivar_name = cultivar_params.get('VRNAME', 'N/A')
        
        # Create horizontal parameter string
        cultivar_text = f"Cultivar: {cultivar_code} ({cultivar_name}) | "
        cultivar_text += f"VSEN={format_param(cultivar_params.get('VSEN'), '.2f')} | "
        cultivar_text += f"PPSEN={format_param(cultivar_params.get('PPSEN'), '.2f')} | "
        cultivar_text += f"P1={format_param(cultivar_params.get('P1'), '.1f')} | "
        cultivar_text += f"P5={format_param(cultivar_params.get('P5'), '.1f')} | "
        cultivar_text += f"PHINT={format_param(cultivar_params.get('PHINT'), '.1f')} | "
        cultivar_text += f"GRNO={format_param(cultivar_params.get('GRNO'), '.1f')} | "
        cultivar_text += f"MXFIL={format_param(cultivar_params.get('MXFIL'), '.2f')} | "
        cultivar_text += f"STMMX={format_param(cultivar_params.get('STMMX'), '.2f')} | "
        cultivar_text += f"SLAP1={format_param(cultivar_params.get('SLAP1'), '.1f')}"
        
        # Add text box at the top-left corner
        fig.text(0.01, 0.998, cultivar_text, 
                fontsize=7, ha='left', va='top',
                bbox=dict(boxstyle='round', facecolor='lightblue', alpha=0.8, pad=0.3),
                family='monospace')
        
        # Determine wheat type based on year
        wheat_type = "Winter Wheat" if EXPERIMENT_YEAR == 2017 else "Spring Wheat"
        # Keep main title at original position
        fig.suptitle(f'DSSAT N-Wheat Crop Growth Model Simulation - Duernast {wheat_type} Experiment {EXPERIMENT_YEAR} under Different Nitrogen Treatments', 
    ...
    plot_configs = [
        ...
        {'idx': 10, 'var': 'grain_protein', 'title': 'k) Grain Protein (lines=simulated, circles=calculated from observed N)', 'ylabel': 'Grain Protein (%)'},
    ...
    for config in plot_configs:
        ...
        if var == 'weather':
            ...
            if nitrogen_data and treatments_data:
                ...
                # Add calculated grain protein points at harvest (calculated from observed grain N)
                # Note: No direct observed protein data - calculated as (observed GNAD / simulated yield) * 100 * 5.75
                if observed_data and consensus_stages:
                    harvest_das = consensus_stages.get('harvest_das', 160)
                    for trt_name in nitrogen_data.keys():
                        if trt_name in observed_data and 'grain_nitrogen' in observed_data[trt_name]:
                            if trt_name in treatments_data:
                                ...
                                obs_grain_n = observed_data[trt_name]['grain_nitrogen']  # Observed GNAD from .WHT
                                ...
                                if sim_grain_yield and sim_grain_yield > 0:
                                    # Calculate protein from observed grain N and simulated yield
                    ...
                    ax.scatter([], [], color='none', marker='o', s=80, 
                             edgecolors='black', linewidth=1.5,
                             label='Protein calculated from observed N (circles)', alpha=0.9)
...
def main():
    ...
    print("[6/6] Reading cultivar parameters...")
    cultivar_params = read_cultivar_parameters()
    if cultivar_params:
        print(f"[OK] Loaded cultivar parameters for {cultivar_params.get('VAR#', 'Unknown')}")
    else:
        print("[WARNING] Could not load cultivar parameters, visualization will proceed without them")
    
    print("[7/7] Creating comprehensive visualization...")
    fig = create_comprehensive_visualization(
        treatments_data, phenology_stages, consensus_stages,
        weather_data, nitrogen_data, observed_data, n_levels, cultivar_params
    )

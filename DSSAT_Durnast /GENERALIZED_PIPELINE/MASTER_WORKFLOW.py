...
class GeneralizedDuernastWorkflowManager:
    ...
    def run_dssat_simulation(self):
        ...
        try:
            ...
            for filename in input_files:
                ...
                if src.exists():
                    ...
                    if dst.exists():
                        dst.unlink()
                    shutil.copy2(src, dst)
                    # Verify copy was successful (especially for weather files)
                    if filename.endswith('.WTH'):
                        ...
                        if src_size == dst_size and abs(src_mtime - dst_mtime) < 2:
                            print(f"  [OK] Copied {filename} (overwrote existing if present) - Verified: {src_size} bytes")
                        else:
                            print(f"  [WARNING] {filename} copy verification failed - sizes: src={src_size}, dst={dst_size}")
                    else:
            ...
            # IMPORTANT: DSSAT reads genotype files from C:\DSSAT48\GENOTYPE\ by default
            # We need to copy our local genotype files to the global directory temporarily
            # OR modify DATA.CDE/DETAIL.CDE to use local paths
            # For now, we'll copy to global directory as a workaround
            print("\nPreparing genotype files for DSSAT...")
            global_genotype_dir = Path('C:/DSSAT48/GENOTYPE')
            if global_genotype_dir.exists():
                try:
                    # Backup original files if they exist
                    backup_dir = global_genotype_dir / 'BACKUP'
                    backup_dir.mkdir(exist_ok=True)
                    
                    # Copy local genotype files to global directory
                    local_genotype = output_dir / 'Genotype'
                    if local_genotype.exists():
                        import shutil
                        for file in local_genotype.glob('WHAPS048.*'):
                            global_file = global_genotype_dir / file.name
                            ...
                            # Copy local file to global directory
                            shutil.copy2(file, global_file)
                            print(f"  [OK] Copied {file.name} to global Genotype directory")
                        print(f"  [INFO] DSSAT will now use your local genotype parameters")
                    else:
                        print(f"  [WARNING] Local Genotype directory not found")
                except Exception as e:
                    print(f"  [WARNING] Could not copy to global Genotype directory: {e}")
                    print(f"  [INFO] You need to manually edit C:\\DSSAT48\\GENOTYPE\\WHAPS048.CUL")
            else:
                print(f"  [WARNING] Global Genotype directory not found at {global_genotype_dir}")
                print(f"  [INFO] DSSAT may not find genotype files - check DATA.CDE configuration")
            
            # Verify weather file is in output directory (DSSAT reads from current working directory)
            weather_file = output_dir / f'{self.config.file_prefix}.WTH'
            if weather_file.exists():
                weather_size = weather_file.stat().st_size
                weather_mtime = datetime.fromtimestamp(weather_file.stat().st_mtime).strftime('%Y-%m-%d %H:%M:%S')
                print(f"\n[VERIFY] Weather file ready in output directory:")
                print(f"  File: {weather_file.name}")
                print(f"  Size: {weather_size:,} bytes")
                print(f"  Modified: {weather_mtime}")
                print(f"  [INFO] DSSAT will read from output directory (current working directory)")
            else:
                print(f"\n[ERROR] Weather file NOT found in output directory: {weather_file}")
                print(f"  [INFO] DSSAT will fail or use wrong weather data!")
            
            # IMPORTANT: Temporarily rename global weather file to force DSSAT to use local one
            # DSSAT may check C:\DSSAT48\Weather\ first, so we disable it temporarily
            global_weather_dir = Path('C:/DSSAT48/Weather')
            global_weather_file = global_weather_dir / f'{self.config.file_prefix}.WTH'
            global_weather_backup = None
            weather_file_disabled = False
            
            if global_weather_file.exists() and weather_file.exists():
                try:
                    # Create backup name with timestamp
                    backup_name = f'{self.config.file_prefix}.WTH.backup_{datetime.now().strftime("%Y%m%d_%H%M%S")}'
                    global_weather_backup = global_weather_dir / backup_name
                    
                    # Rename global file to backup (temporarily disable it)
                    global_weather_file.rename(global_weather_backup)
                    weather_file_disabled = True
                    print(f"\n[INFO] Temporarily disabled global weather file:")
                    print(f"  Renamed: {global_weather_file.name} -> {backup_name}")
                    print(f"  [INFO] DSSAT will now use weather file from output directory")
                except Exception as e:
                    print(f"\n[WARNING] Could not disable global weather file: {e}")
                    print(f"  [INFO] DSSAT may still use global weather file from C:\\DSSAT48\\Weather\\")
            ...
            try:
                ...
                # Double-check weather file is accessible from output directory
                if not Path(f'{self.config.file_prefix}.WTH').exists():
                    print(f"  [ERROR] Weather file {self.config.file_prefix}.WTH not found in output directory!")
                    print(f"  [INFO] Current directory: {os.getcwd()}")
                    print(f"  [INFO] Files in output directory: {list(Path('.').glob('*.WTH'))}")
                ...
                # Restore global weather file if we disabled it
                if weather_file_disabled and global_weather_backup and global_weather_backup.exists():
                    try:
                        global_weather_backup.rename(global_weather_file)
                        print(f"\n[INFO] Restored global weather file: {global_weather_file.name}")
                    except Exception as e:
                        print(f"\n[WARNING] Could not restore global weather file: {e}")
                        print(f"  [INFO] Backup file remains at: {global_weather_backup}")
  ...
  python MASTER_WORKFLOW.py --all       # Run all available experiments
    ...
    if args.list:
        ...
        for year in available:
            ...
            print(f"  {year}: {config.experiment_dir} ({config.crop_type})")
        return 0

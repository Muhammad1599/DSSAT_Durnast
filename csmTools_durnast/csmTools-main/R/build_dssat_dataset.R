...
build_dssat_dataset <-  function(dataset) {
  # Normalize: if dataset has GENERAL (from apply_naming_rules) but not MANAGEMENT, wrap into MANAGEMENT
  mngt_secs <- c("GENERAL", "TREATMENTS", "CULTIVARS", "FIELDS", "INITIAL_CONDITIONS", "SOIL_ANALYSIS",
                 "PLANTING_DETAILS", "IRRIGATION", "FERTILIZERS", "RESIDUES", "CHEMICALS", "TILLAGE",
                 "ENVIRONMENT_MODIFICATIONS", "HARVEST", "SIMULATION_CONTROLS")
  if ("GENERAL" %in% names(dataset) && !"MANAGEMENT" %in% names(dataset)) {
    present_mngt <- intersect(mngt_secs, names(dataset))
    mngt_list <- dataset[present_mngt]
    if (is.data.frame(mngt_list[["GENERAL"]]) && "file_name" %in% names(mngt_list[["GENERAL"]])) {
      attr(mngt_list, "file_name") <- unique(mngt_list[["GENERAL"]]$file_name)
    }
    dataset <- c(list(MANAGEMENT = mngt_list), dataset[setdiff(names(dataset), present_mngt)])
  }
  data_fmt <- dataset %>%
    purrr::imap(~{
      switch(

...
.apply_mapping_rules <- function(input_data, rules, ...) {
  ...
  if (length(transformation_rules) == 0) {
    ...
    for (rule in transformation_rules) {
      src_sec <- rule$source$section
      tgt_sec <- if (is.list(rule$target)) rule$target$section else rule$target_section
      # Resolve input: explicit source, or for aggregate rules (no source) use target from output
      if (is.null(src_sec) || length(src_sec) == 0 || (is.character(src_sec) && trimws(src_sec) == "")) {
        if (!is.null(tgt_sec) && tgt_sec %in% names(output_data)) {
          input_df <- output_data[[tgt_sec]]
        } else {
          next
        }
      } else if (src_sec %in% names(output_data)) {
        input_df <- output_data[[src_sec]]
      } else if (src_sec %in% names(input_data)) {
        input_df <- input_data[[src_sec]]
      } else {
        next
      }

...
.impute_soil_metadata <- function(soil_data, exp_metadata) {
  ...
  soil_data %>%
    ...
    dplyr::mutate(
      ...
      YEAR = dplyr::coalesce(YEAR, YEAR_FROM_DATE, as.character(EXP_YEAR)),
...
.format_mngt_locations <- function(mngt_data_locations) {
  ...
  mngt_data_locations %>%
    dplyr::mutate(
      ...
      FLNAME = ifelse(is.na(FLNAME), if ("ShortLabel" %in% names(.)) toupper(ShortLabel) else NA_character_, FLNAME),
      # Fill site as address if missing (use address columns only if present)
      SITE = if ("SITE" %in% names(.)) SITE else NA_character_,
      SITE_addr = {
        addr_cols <- intersect(c("FLNAME", "City", "Region", "CntryName"), names(.))
        if (length(addr_cols) > 0) {
          tidyr::unite(dplyr::select(., dplyr::all_of(addr_cols)), "SITE", sep = ", ", na.rm = TRUE)$SITE
        } else {
          if ("FLNAME" %in% names(.)) FLNAME else rep(NA_character_, dplyr::n())
        }
      },
  ...
  wth_data_locations %>%
    ...
    dplyr::mutate(
      ...
      WST_NAME = dplyr::coalesce(WST_NAME, {
        addr_cols <- intersect(c("ShortLabel", "City", "Region", "CntryName"), names(.))
        if (length(addr_cols) > 0) {
          tidyr::unite(dplyr::select(., dplyr::all_of(addr_cols)), "WST_SITE", sep = ", ", na.rm = TRUE)$WST_SITE
        } else {
